// Vision Loop Playwright Script - IMPROVED VERSION
...
const REPORT_FILE = path.join(SCREENSHOT_DIR, 'vision-loop-report-v2.md');
...
function analyzeScreenshot(imagePath) {
  ...
  // Reduced threshold from 5000 to 1000 bytes
  if (stats.size < 1000) {
    return { 
      valid: false, 
...
async function runVisionLoop() {
  ...
  console.log('Starting Vision Loop QA (v2)...');
  ...
  for (const viewport of VIEWPORTS) {
    ...
    for (const pageConfig of PAGES) {
      ...
      try {
        // Increased timeout from 10000 to 30000
        // Changed waitUntil to 'load' instead of 'networkidle' for faster loading
        ...
        await page.screenshot({ 
          path: filePath, 
          fullPage: true,  // Changed to fullPage
          type: 'png'
        });
  ...
  console.log('VISION LOOP COMPLETE (v2)');
  console.log('='.repeat(60));
...
function generateReport(results) {
  let markdown = '# Vision Loop Report (v2)\n\n';
